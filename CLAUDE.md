...
- **Favicons:** regenerated crisp favicons from the **real logo art** via `script/make_favicons.py` (pure-stdlib, no Pillow). The script decodes `logo-transparent.png`, isolates the globe mark (globe + orbital ring + satellite, excluding the wordmark) via a fixed crop box, and renders it square/centered on the brand-dark `#0f0f12` background. Replaced the blurry 64px `favicon.png`/`favicon.ico` with a sharp 256px PNG + 16/32/48 ICO, added `apple-touch-icon.png` (180px). Matches the GlobalReach iOS app icon. (An earlier pass used a hand-drawn wireframe globe + `favicon.svg`; both removed — that wasn't the real logo.)
